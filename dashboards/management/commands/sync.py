...
class Command(BaseCommand):
    help = "Calls sync tasks to sync data. Example usage:" + \
           "\"python manage.py resync -verbosity=2 -u 26 -i google\" will call a sync task for user 26's google data"

    def sync(self, integration, user, verbosity=1):
        """
            Starts celery task to sync user integration data
        Args:
        ...
        """
        if verbosity >= 2:
            self.stdout.write(f"Syncing {integration.provider} for user {user.id}")
    ...
    def handle(self, *args, **options):
        ...
        for user in users:
            ...
            for account in accounts:
                self.sync(account, user, options["verbosity"])
